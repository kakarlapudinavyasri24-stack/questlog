...
function App() {
  ...
  async function fetchTasks() {
    try {
      ...
      if (!response.ok) throw new Error("Could not load quests.");
  ...
  function evaluateBadges(nextGame, nextTasks) {
    ...
    const hasType = (questType) => nextTasks.some((task) => task.type === questType);
    const completedType = (questType) => completed.some((task) => task.type === questType);
    ...
    const newBadges = [...earned].filter((badge) => !nextGame.badges.includes(badge));
  ...
  function commitGame(nextGame, nextTasks) {
    ...
    if (newBadges.length > 0) {
      const badge = BADGES.find((item) => item.id === newBadges[0]);
      setBadgeModal(badge);
    }
  }
  ...
  async function addTask(event) {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/tasks`, {
        ...
        body: JSON.stringify({ title: trimmedTitle, type })
      });

      if (!response.ok) throw new Error("Could not add quest.");
  ...
  async function updateTaskStatus(id, status) {
    ...
    headers: { "Content-Type": "application/json" },
      ...
      if (!response.ok) throw new Error("Could not update quest.");
      const updatedTask = await response.json();
      const nextTasks = tasks.map((item) => (item.id === id ? updatedTask : item));
      const previousLevel = getLevelInfo(game.xp).index;
      ...
      if (nextLevel.index > previousLevel) {
        setLevelModal(nextLevel);
      }
    } catch (err) {
      setError(err.message);
    }
  ...
  async function generateChronicle() {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/chronicle`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          tasks,
      ...
      if (!response.ok) throw new Error("Could not generate the Daily Chronicle.");
      const data = await response.json();
  ...
  return (
    <main className={`game-shell level-${level.index}`}>
      <section className="top-bar">
        <div>
          <p className="pixel-kicker">{weather.join(" ")}</p>
          <h1>⚔️ Questlog</h1>
          <p className="subtitle">Pixel task RPG for daily quests</p>
        </div>
      ...
      <section className="hud-grid">
        ...
        <article className="hud-card">
          ...
          <small>{level.index === LEVELS.length - 1 ? "Max title reached" : `${level.nextXp - game.xp} XP to ${level.nextTitle}`}</small>
        </article>
      ...
      {activeTab === "quests" ? (
        <>
          <form className="quest-form" onSubmit={addTask}>
            ...
            />
            <select aria-label="Quest type" value={type} onChange={(event) => setType(event.target.value)}>
          ...
          <section className="content-grid">
            ...
            <aside className="chronicle-panel">
              ...
              <h2>End Day</h2>
              <p>
                Uses HP, level, streak, completion rate, and quest outcomes to set the narrator tone.
              </p>

         <button type="button" onClick={generateChronicle} disabled={isLoading}>
                {isLoading ? "Writing..." : "End Day → Generate Chronicle"}
              </button>
          ...
          <form className="modal-card settings-card" onSubmit={saveSettings}>
            ...
            <label className="toggle-row">
              <input
                ...
                onChange={(event) => setDraftSettings({ ...draftSettings, demoMode: event.target.checked })}
              />
              Use mock chronicle
            </label>
